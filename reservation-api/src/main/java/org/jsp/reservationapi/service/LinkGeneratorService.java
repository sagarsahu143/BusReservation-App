...
public class LinkGeneratorService {
	...
	public String getActivationLink(Admin admin, HttpServletRequest request) {
		...
		String sitrurl = request.getRequestURL().toString();
		return sitrurl.replace(request.getServletPath(), ADMIN_VERIFY_LINK + admin.getToken());
	}
	
	public String getActivationLink(User  user, HttpServletRequest request) {
		user.setToken(RandomString.make(10));
		userDao.saveUser(user);
		String sitrurl = request.getRequestURL().toString();
		return sitrurl.replace(request.getServletPath(),USER_VERIFY_LINK + user.getToken());
	}
	...
	public String getResetPasswordLink(Admin admin, HttpServletRequest request) {
		...
		adminDao.saveAdmin(admin);
		String sitrurl = request.getRequestURL().toString();
		return sitrurl.replace(request.getServletPath(), ADMIN_RESET_PASSWORD_LINK + admin.getToken());
	}
	...
	public String getResetPasswordLink(User user, HttpServletRequest request) {
		user.setToken(RandomString.make(10));
	     userDao.saveUser(user);
		String sitrurl = request.getRequestURL().toString();
		return sitrurl.replace(request.getServletPath(), USER_RESET_PASSWORD_LINK + user.getToken());
	}
